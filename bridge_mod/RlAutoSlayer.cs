...
public class RlAutoSlayer
{
    ...
    public void Start(string seed, string? logFile = null)
    {
        ...
        // §14: headful mode drives NATIVE selection UIs through the
        // coordinator (no CardSelectCmd selector bypass). Headless keeps
        // the legacy selector.
        if (HeadfulMode)
        {
            TaskHelper.RunSafely(
                RlNativeSelectionCoordinator.RunLoopAsync(_cts.Token));
        }
    }
