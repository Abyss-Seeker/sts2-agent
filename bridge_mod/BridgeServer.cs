...
public class BridgeServer
{
    ...
    /// </summary>
    public static bool AllowRandomFallback = true;
    ...
    private static string EnrichWithRunSummary(string stateJson)
    {
        try
        {
            ...
            if (node == null || node["type"] == null || node["player"] != null)
                return stateJson;
    ...
    private async Task HandleStartRunAsync(JsonElement root)
    {
        ...
        try
        {
            var slayer = RlAutoSlayer.Active;
            if (slayer == null)
            {
                SendState(RlAutoSlayer.BuildStartRunAck(
                    seed, character, difficulty, false, "no active AutoSlayer"));
                return;
            }
