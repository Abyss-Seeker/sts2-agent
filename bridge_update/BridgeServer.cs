...
public class BridgeServer
{
    ...
    private static string EnrichWithRunSummary(string stateJson)
    {
        try
        {
            ...
            if (playerSummary != null && playerSummary.TryGetValue("potions", out var belt))
                node["potions"] = JsonSerializer.SerializeToNode(belt);
            if (type != "combat_action" && playerSummary != null)
    ...
    public async Task<string?> SendStateAndWaitForActionAsync(
        ...
        => await SendStateAndWaitForActionAsync(() => stateJson, timeout, ct);

    public async Task<string?> SendStateAndWaitForActionAsync(
        Func<string> snapshot, TimeSpan timeout, CancellationToken ct)
    {
        while (true)
        {
            string stateJson = snapshot();
            string? response = await SendStateAndWaitCoreAsync(stateJson, timeout, ct);
            if (response == null) return null;
            using var command = JsonDocument.Parse(response);
            var root = command.RootElement;
            string action = root.GetProperty("action").GetString() ?? "";
            using var state = JsonDocument.Parse(stateJson);
            bool combat = state.RootElement.GetProperty("type").GetString() == "combat_action";
            if (action != "discard_potion" && (action != "potion" || combat)) return response;
            string? requestId = root.TryGetProperty("request_id", out var id) ? id.ToString() : null;
            bool accepted = await PotionActions.Execute(root, ct);
            RecordActionResult(requestId, accepted, accepted ? "potion command accepted by native queue" : "potion no longer legal");
            // A new authoritative state follows every belt action. Room choice
            // indices are never consumed by a potion or replaced by random play.
        }
    }

    private async Task<string?> SendStateAndWaitCoreAsync(
        string stateJson, TimeSpan timeout, CancellationToken ct)
    {
